...
| คำสั่ง | การทำงาน | เหตุผลที่ใช้ |
|---------|------------|---------------|
| `docker-compose up -d` | สร้างและรัน container ทั้งหมดตามไฟล์ `docker-compose.yml` โดยทำงานแบบ background (detached mode) | เพื่อเริ่มต้น environment สำหรับพัฒนา ESP32 โดยไม่ต้องเปิด terminal ค้างไว้ |
| `docker-compose exec esp32-dev bash` | เปิด shell (bash) ภายใน container ที่ชื่อ **esp32-dev** | เพื่อเข้าไปทำงานภายใน container เช่น build, flash, monitor หรือใช้คำสั่งของ ESP-IDF |

>  **สรุป:**  
> `docker-compose up -d` ใช้สร้าง/รัน container  
> `docker-compose exec esp32-dev bash` ใช้เข้าไปทำงานภายใน container

---

##  2. ESP-IDF Tools (จาก Lab4)

เครื่องมือหลักที่ใช้คือ **ESP-IDF (Espressif IoT Development Framework)**  
โดยใช้คำสั่งหลักดังนี้:

| คำสั่ง | หน้าที่ |
|---------|----------|
| `idf.py build` | คอมไพล์โค้ด C/C++ ให้เป็น binary สำหรับ ESP32 |
| `idf.py flash` | อัปโหลดไฟล์ binary ไปยังบอร์ด ESP32 ผ่านพอร์ต USB |
| `idf.py monitor` | เปิด serial monitor เพื่อดูข้อความจาก ESP32 |
| `idf.py flash monitor` | รวมขั้นตอน flash และ monitor เข้าด้วยกัน |

> เครื่องมือ `idf.py` เป็น command-line tool ของ ESP-IDF  
> ใช้สำหรับ build, flash และ monitor โปรแกรม ESP32 ได้ครบวงจร

---

##  3. New Tools (tree, htop)

| เครื่องมือ | หน้าที่ | ตัวอย่างการใช้งาน |
|-------------|----------|--------------------|
| `tree` | แสดงโครงสร้างโฟลเดอร์และไฟล์ในรูปแบบต้นไม้ | `tree` → แสดงโครงสร้างโปรเจกต์ ESP-IDF |
| `htop` | แสดงการใช้ CPU, RAM และ process ต่าง ๆ แบบเรียลไทม์ (interactive system monitor) | `htop` → ดูว่า container หรือระบบใช้ทรัพยากรเท่าไร |

>  **สรุป:**  
> `tree` ใช้ดูโครงสร้างไฟล์  
> `htop` ใช้ตรวจสอบประสิทธิภาพของระบบภายใน container

---

##  4. Architecture Focus

| ด้านเปรียบเทียบ | **Lab4: Arithmetic** | **Lab5: ESP32 Architecture** |
|------------------|-----------------------|------------------------------|
| **เนื้อหา/เป้าหมาย** | ฝึกการคำนวณเชิงตรรกะ เช่น addition, subtraction, multiplication | ศึกษาโครงสร้างภายในของ ESP32 เช่น CPU core, memory, GPIO, Wi-Fi, Bluetooth |
| **ระดับการศึกษา** | เน้นระดับ **software / instruction** | เน้นระดับ **hardware / architecture** |
| **เครื่องมือหลัก** | คอมไพเลอร์ และ `idf.py` เพื่อรันโค้ดคำนวณ | Datasheet, block diagram, และ ESP-IDF API |
| **จุดประสงค์หลัก** | เรียนรู้การควบคุม CPU ให้คำนวณได้ถูกต้อง | เข้าใจระบบภายในและส่วนประกอบของ ESP32 |

>  **สรุป:**  
> - Lab4 → เน้น “การคำนวณและโปรแกรม”  
> - Lab5 → เน้น “เข้าใจสถาปัตยกรรมและการทำงานของชิปจริง”
...
|----------------|-------------------|----------------------|-------------|
| Stack | `stack_var` | `0x3ffb8f10` | Internal SRAM |
| Global SRAM | `sram_buffer` | `0x3ffc10a0` | Internal SRAM |
| Flash | `flash_string` | `0x400d1524` | Flash (.rodata) |
| Heap | `heap_ptr` | `0x3ffc55f0` | Internal SRAM (dynamic allocation) |

**Table 2.2: Memory Usage Summary**

| Memory Type | Free Size (bytes) | Total Size (bytes) |
|-------------|-------------------|--------------------|
| Internal SRAM | 320,512 | 520,192 |
| Flash Memory | – (read-only) | ขึ้นกับขนาดโปรแกรม |
| DMA Memory | 214,784 | varies (subset of SRAM) |

##  คำถามวิเคราะห์ (ง่าย) 

| ข้อ | คำตอบ |
|---------|----------------------------|
| **1.Memory Types** | - **SRAM** เก็บข้อมูลที่เปลี่ยนแปลงได้ เช่น ตัวแปรใน **stack**, **global**, **heap**<br> - **Flash Memory** เก็บข้อมูลถาวร เช่น โค้ดโปรแกรม และข้อความคงที่ (`const`, string) |
| **2.Address Ranges** | - **SRAM:** `0x3FFB0000 – 0x3FFDFFFF`<br> - **Flash:** `0x400D0000 – 0x40400000` |
| **3.Memory Usage** | - **SRAM รวม:** ~520 KB (ใช้งานได้จริง ~320 KB)<br> - **Flash รวม:** ~4 MB (เก็บโค้ดและข้อมูลถาวร)<br> - **SPI RAM:** ไม่มีใน QEMU (0 bytes) |

>  **สรุป:**  
> ตัวแปรที่แก้ไขได้อยู่ใน **SRAM**,  
> ข้อมูลถาวรอยู่ใน **Flash**,  
> ESP32 มีหน่วยความจำโดยรวม **SRAM ~520KB** และ **Flash ~4MB**
...
|------------|-----------------|------------|------------------|
| Sequential | Internal SRAM    | 6800       | 1.00x            |
| Random     | Internal SRAM    | 8100       | 1.19x            |
| Sequential | External Memory  | 29500      | 1.03x            |
| Random     | External Memory  | 31200      | 1.05x            |
...
1. **Cache Efficiency**: ทำไม sequential access เร็วกว่า random access?
  - การเข้าถึงแบบ Sequential เร็วกว่า Random เพราะ cache สามารถโหลดข้อมูลล่วงหน้าที่ต่อเนื่องได้ 
2. **Memory Hierarchy**: ความแตกต่างระหว่าง internal SRAM และ external memory คืออะไร?
  - Internal SRAM ใกล้ CPU → latency ต่ำกว่า PSRAM 
3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?
  - Stride ขนาดใหญ่ → เข้าถึงข้อมูลน้อย → เวลาในการทำงานเร็วขึ้น แต่ลดประสิทธิภาพ cache locality
...
| Average Time per Iteration (μs)  | 125            | 98             |
| Total Execution Time (ms)        | 12,500         | 14,700         |
...
| Average Latency (μs) | 240       |
| Queue Overflow Count | 0         |

### คำถามวิเคราะห์

1. **Core Specialization (ความเหมาะสมของ Core):**  
   - **Core 0 (PRO_CPU)** เหมาะกับงานประเภท integer/protocol processing เพราะค่าเฉลี่ยเวลาต่อ iteration สูงกว่าเล็กน้อยและทำงานคำนวณแบบ integer ได้ดี  
   - **Core 1 (APP_CPU)** เหมาะกับงาน floating point หรือการประมวลผลแอปพลิเคชัน เช่น การคำนวณแบบ float-intensive  

2. **Communication Overhead (ค่า Overhead ของการสื่อสารระหว่าง Core):**  
   - ค่า latency เฉลี่ยของ inter-core communication อยู่ประมาณ **240 μs ต่อข้อความ**  
   - ไม่มี queue overflow → การสื่อสารระหว่าง core มี overhead ต่ำและมีประสิทธิภาพดี  

3. **Load Balancing (การกระจายงานระหว่าง Core):**  
   - งานถูกกระจายอย่างสมดุลระหว่าง Core 0 และ Core 1  
   - ทั้งสอง core ทำงานเต็มประสิทธิภาพ → ไม่มีคอขวด  
   - การใช้ delay/pause เหมาะสมช่วยให้ระบบ dual-core ทำงานเสถียรและลดเวลารวมในการประมวลผล  
---
...
**ข้อมูลนักศึกษา:**
- ชื่อ: น.ส.น้ำทิพย์ ป้อมหิน
- รหัสนักศึกษา: 67030115
- วันที่ทำการทดลอง: 19/10/68
...
1. **เปรียบเทียบประสบการณ์การใช้ Docker ในสัปดาห์นี้กับสัปดาห์ที่ 4:**  
   - ในสัปดาห์นี้รู้สึกคล่องตัวขึ้นมากเมื่อเทียบกับสัปดาห์ที่ 4  
   - สามารถสร้างและรัน container สำหรับ ESP32 ได้รวดเร็ว ลดปัญหาเรื่อง dependency และ environment mismatch  
   - การจัดการ container (เช่น เข้าไป build project, mount volume) ทำได้ง่ายและเร็วขึ้น  

2. **สิ่งที่เรียนรู้เพิ่มเติมเกี่ยวกับ ESP32 Architecture:**  
   - เข้าใจการทำงานแบบ dual-core ของ ESP32 และความแตกต่างระหว่าง PRO_CPU และ APP_CPU  
   - การเข้าถึงหน่วยความจำ SRAM vs PSRAM ส่งผลต่อเวลาในการอ่าน/เขียนข้อมูล และ cache locality มีผลต่อ performance  
   - การสื่อสารระหว่าง cores (inter-core communication) มีค่า latency และสามารถใช้ queue/semphr จัดการ synchronization ได้  

3. **ความท้าทายที่พบในการทำ Architecture Analysis:**  
   - การออกแบบ task ให้สมดุลระหว่าง cores (load balancing) ต้องคำนวณ delay และ priority ให้เหมาะสม  
   - การวัดค่า latency และเวลาเฉลี่ยต่อ iteration ต้องทำอย่างระมัดระวังเพื่อให้ได้ข้อมูลใกล้เคียงความเป็นจริง  
   - การวิเคราะห์ผลจาก cache, stride, และ dual-core interaction ต้องตีความอย่างรอบคอบเพื่อไม่ให้สรุปผิดพลาด
